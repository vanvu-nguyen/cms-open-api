...
public class CommonOpenPGPUtils {
    ...
    static void copyAsLiteralData(OutputStream outputStream, InputStream in, long length, int bufferSize) {
        ...
        byte[] buff = null;
        try {
            pOut = lData.open(outputStream, PGPLiteralData.BINARY, PGPLiteralData.CONSOLE,
                    java.util.Date.from(LocalDateTime.now().toInstant(ZoneOffset.UTC)), new byte[bufferSize]);
            ...
            throw new RuntimeException("Copy As Literal Data failed.");
        } finally {
            // Clearing buffer
            Arrays.fill(Objects.requireNonNull(buff), (byte) 0);
            try {
                pOut.close();
